...
public class AutonomousRightOfStation extends CommandBase {
  ...
  public AutonomousRightOfStation(DrivebaseSubsystem subsystem, GrabberSubsystem grabbersubsystem, ElevatorSubsystem elevatorsubsystem, ArmSubsystem armsubsystem, LimelightSubsystem limelightsubsystem) {
  ...
  public void execute() {
    ...
    if (currentMove == 9) {
      doingNinthMove = drivebasesub.turnXdegrees(270);
      if (!doingNinthMove) {
        currentMove++;
        resetEncoderGyro();
      }
    ...
    if (currentMove == 11) {
      doingEleventhMove = drivebasesub.turnXdegrees(90);
